...
type DiscoverRelationshipsRequest struct {
	...
	Method      string  `json:"method"` // "keyword" (default, fast) or "ai" (slow, requires Ollama)
}
...
// Default method is "keyword" (fast, no AI required). Use {"method": "ai"} for Ollama-based discovery.
func (s *Server) discoverRelationships(c *gin.Context) {
	...
	minScore := req.MinStrength
	if minScore <= 0 {
		minScore = 0.3
	}

	if req.Method == "ai" {
		// AI-based discovery (slow, requires Ollama)
		if s.aiManager == nil {
		...
		// Save suggestions as real relationships
		for _, suggestion := range suggestions {
			_, _ = s.relService.Create(&relationships.CreateOptions{
				SourceMemoryID:   suggestion.SourceID,
				TargetMemoryID:   suggestion.TargetID,
		...
		// Keyword-based discovery (fast, no AI required)
		_, err := s.relService.BatchDiscoverKeyword(limit, minScore, "")
		if err != nil {
			InternalError(c, "Keyword discovery failed: "+err.Error())
			return
		}
	}

	// Return all relationships so the client has a complete picture
	rels, err := s.db.GetAllRelationships(1000, 0)
	if err != nil {
		InternalError(c, "Failed to fetch relationships after discovery: "+err.Error())
		return
	}

	response := make([]RelationshipResponse, len(rels))
...
// BatchDiscoverRequest represents a batch relationship discovery request
...
// GraphStatsResponse represents graph-level statistics
type GraphStatsResponse struct {
	TotalNodes          int                    `json:"total_nodes"`
	TotalEdges          int                    `json:"total_edges"`
	AvgDegree           float64                `json:"avg_degree"`
	OrphanCount         int                    `json:"orphan_count"`
	MostConnected       []database.NodeDegree  `json:"most_connected"`
	RelationshipTypes   map[string]int         `json:"relationship_types"`
	ConnectedComponents int                    `json:"connected_components"`
}

// getAllRelationships handles GET /api/v1/relationships
func (s *Server) getAllRelationships(c *gin.Context) {
	limit := parseIntQuery(c, "limit", 1000)
	minStrength := parseFloatQuery(c, "min_strength", 0)

	rels, err := s.db.GetAllRelationships(limit, minStrength)
	if err != nil {
		InternalError(c, "Failed to get relationships: "+err.Error())
		return
	}
	...
	SuccessResponse(c, "Found "+intToString(len(response))+" relationships", response)
}

// batchDiscoverRelationships handles POST /api/v1/relationships/batch-discover
func (s *Server) batchDiscoverRelationships(c *gin.Context) {
	var req BatchDiscoverRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		req = BatchDiscoverRequest{}
	}
	...
	created, err := s.relService.BatchDiscoverKeyword(limit, req.MinScore, req.Domain)
	if err != nil {
		InternalError(c, "Batch discover failed: "+err.Error())
		return
	}

	SuccessResponse(c, "Batch discovery completed", map[string]interface{}{
		"relationships_created": created,
	})
}

// getGraphStats handles GET /api/v1/graph/stats
func (s *Server) getGraphStats(c *gin.Context) {
	stats, err := s.db.GetGraphStats()
	if err != nil {
		InternalError(c, "Failed to get graph stats: "+err.Error())
		return
	}
	...
	SuccessResponse(c, "Graph statistics retrieved", response)
}
