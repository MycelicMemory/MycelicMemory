...
func (s *Server) getGraph(c *gin.Context) {
	...
	// Get full memory data for central memory and all nodes
	centralMem, err := s.db.GetMemory(memoryID)
	if err != nil || centralMem == nil {
		NotFoundError(c, "Memory not found")
		return
	}

	// Build a map of memory ID to full memory data
	memoryMap := make(map[string]*MemoryData)
	for _, n := range result.Nodes {
		mem, _ := s.db.GetMemory(n.ID)
		if mem != nil {
			memoryMap[n.ID] = toMemoryData(mem)
		}
	}

	// Get full relationships for edges
	relationshipMap := make(map[string][]GraphNodeRelationship)
	edges := make([]GraphEdge, 0, len(result.Edges))

	for _, e := range result.Edges {
		// Get full relationship info from database
		rels, _ := s.db.GetRelationshipsBetween(e.SourceID, e.TargetID)
		for _, rel := range rels {
			...
			relationshipMap[e.SourceID] = append(relationshipMap[e.SourceID], nodeRel)
			if e.SourceID != e.TargetID {
				relationshipMap[e.TargetID] = append(relationshipMap[e.TargetID], nodeRel)
			}
		}
	}
	...
	response := &GraphResponse{
		CentralMemory: toMemoryData(centralMem),
		Depth:         depth,
		Edges:         edges,
		Nodes:         nodes,
