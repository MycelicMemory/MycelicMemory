...
func (s *Server) handleStoreMemory(ctx context.Context, argsJSON []byte) (interface{}, error) {
	...
	// Auto-create relationships to similar existing memories
	go s.autoCreateRelationships(result.Memory)
...
// autoCreateRelationships finds similar existing memories and creates relationships.
// Runs asynchronously to avoid blocking the store_memory response.
func (s *Server) autoCreateRelationships(mem *database.Memory) {
	...
	if err != nil {
		s.log.Debug("auto-relationship search failed", "error", err)
		return
	}
	...
	for _, result := range results {
		...
		rel := &database.Relationship{
			...
			Context:          "Auto-discovered on store",
			AutoGenerated:    true,
		}
	...
	if created > 0 {
		s.log.Debug("auto-created relationships", "memory_id", mem.ID, "count", created)
	}
}
...
func (s *Server) handleLinkSessionMemories(ctx context.Context, argsJSON []byte) (interface{}, error) {
	var params LinkSessionMemoriesParams
	if err := json.Unmarshal(argsJSON, &params); err != nil {
		return nil, fmt.Errorf("invalid parameters: %w", err)
	}
	...
	linked, err := s.claudeIngester.LinkSessionMemories(params.ConversationID)
	if err != nil {
		return nil, fmt.Errorf("failed to link session memories: %w", err)
	}
