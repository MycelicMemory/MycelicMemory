...
// GetAllRelationships retrieves all relationships with optional filters
func (d *Database) GetAllRelationships(limit int, minStrength float64) ([]*Relationship, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
...
// GraphStats contains graph-level statistics
...
// NodeDegree represents a node and its connection count
type NodeDegree struct {
	MemoryID string `json:"memory_id"`
	Degree   int    `json:"degree"`
}

// GetGraphStats computes graph-level statistics
func (d *Database) GetGraphStats() (*GraphStats, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	...
	// Total nodes (memories)
	d.db.QueryRow(`SELECT COUNT(*) FROM memories`).Scan(&stats.TotalNodes)

	// Total edges
	d.db.QueryRow(`SELECT COUNT(*) FROM memory_relationships`).Scan(&stats.TotalEdges)
	...
	// Orphan count (memories with no relationships)
	d.db.QueryRow(`
		SELECT COUNT(*) FROM memories m
		WHERE NOT EXISTS (
			SELECT 1 FROM memory_relationships r
			WHERE r.source_memory_id = m.id OR r.target_memory_id = m.id
		)
	...
	if err == nil {
		...
		for rows.Next() {
			var nd NodeDegree
			if err := rows.Scan(&nd.MemoryID, &nd.Degree); err == nil {
				stats.MostConnected = append(stats.MostConnected, nd)
			}
		}
	}
	...
	if err == nil {
		...
		for typeRows.Next() {
			var relType string
			var count int
			if err := typeRows.Scan(&relType, &count); err == nil {
				stats.RelationshipTypes[relType] = count
			}
		}
	}
...
// GetMemoriesByConversationID retrieves all memories linked to a conversation
func (d *Database) GetMemoriesByConversationID(conversationID string) ([]*Memory, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	...
	`, conversationID)
	if err != nil {
		return nil, fmt.Errorf("failed to get memories by conversation: %w", err)
	}
	defer rows.Close()
...
// GetMemoriesByDomainAndTags retrieves memories in a domain that match at least minSharedTags tags
func (d *Database) GetMemoriesByDomainAndTags(domain string, tags []string, minSharedTags int, limit int) ([][2]string, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	...
	// Get all memories in domain
	rows, err := d.db.Query(`
		SELECT id, tags FROM memories WHERE domain = ? AND tags IS NOT NULL AND tags != '[]'
	`, domain)
	if err != nil {
		return nil, fmt.Errorf("failed to query memories: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := json.Unmarshal([]byte(tagsJSON), &parsedTags); err != nil {
			continue
		}
		...
		mems = append(mems, memTags{id: id, tags: tagSet})
	}

	// Find pairs with enough shared tags
	var pairs [][2]string
	for i := 0; i < len(mems) && len(pairs) < limit; i++ {
		for j := i + 1; j < len(mems) && len(pairs) < limit; j++ {
			shared := 0
			for tag := range mems[i].tags {
				if mems[j].tags[tag] {
					shared++
				}
			}
			if shared >= minSharedTags {
				pairs = append(pairs, [2]string{mems[i].id, mems[j].id})
			}
		}
	}
